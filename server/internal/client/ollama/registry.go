...
func (r *trackingReader) Read(p []byte) (n int, err error) {
	n, err = r.r.Read(p)
	recv := r.received.Add(int64(n))
	r.update(r.l, recv, nil)
	return
}
...
func (r *Registry) Pull(ctx context.Context, name string) error {
		...
		func() {
			...
			for cs, err := range r.chunksums(ctx, name, l) {
				...
				if err == nil {
					...
					t.update(l, received.Load(), ErrCached)
					continue
				}
				...
				g.Go(func() (err error) {
					...
					body := &trackingReader{l: l, r: res.Body, received: &received, update: t.update}
	...
	if recv := received.Load(); recv != expected {
